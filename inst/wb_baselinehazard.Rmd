...
```
## Calculating the Weibull-Parameters upfront (No Data in the Cox-Model Needed)
This is not easily possible because FOE's functions need the data to get strata.
...
We use the new function "Wb_GetInfo" to extract the needed information from the
Cox-model and the original data
```{r}
source(paste0(here(), "/Wb_GetInfo.R"))
wb_info <- Wb_GetInfo(
  cox_fit,
  timestart = df$age,
  timestop = df$eventage,
...
object retrieved via "Wb_GetInfo"
```{r}
# Predict event-probabilities
source(paste0(here(), "/Wb_EventProb_nodata.R"))  
